...
# Lab 3 (Python-Spur): Client-Stub generieren
...
`host.docker.internal` löst den Host-Rechner aus dem Container heraus auf und funktioniert so auf Docker Desktop (Windows/Mac). Unter nativem Docker Engine (häufig unter Linux) muss die Zuordnung explizit ergänzt werden:
...
Alternativ kann unter Linux `--network=host` verwendet und die Quelle direkt als `http://localhost:8081/openapi.json` angegeben werden.
...
Die Qualität des generierten Clients hängt direkt von der Qualität der Spezifikation ab: fehlen `summary`/`description` oder `responses`-Angaben (siehe Lab 1), fehlen diese auch im generierten Client - der Generator kann nur wiedergeben, was in der Spezifikation steht.
