# Übung (Python-Spur): Docker-Compose-Datei erstellen
...
2. **Umgebungsvariablen ergänzen.** Prüft, dass `RABBITMQ_HOST` bei beiden Python-Services auf `rabbitmq` (den Servicenamen) zeigt, nicht auf `localhost` oder `host.docker.internal`.

3. **Healthcheck für Order-Service ergänzen.** Fügt dem `order-service` einen Healthcheck über `/health` hinzu, analog zum RabbitMQ-Healthcheck aus der Theorie (Werkzeug: `python -c` mit `urllib` reicht, da `curl`/`wget` im `python:3.11-slim`-Image nicht vorinstalliert sind).
