# Übung (Python-Spur): Deployment-Strategien, DevOps und Abschlussdemonstration
...
2. **CI/CD-Pipeline skizzieren.** Skizziert (als Liste oder einfaches Diagramm) die Schritte einer CI/CD-Pipeline für den Order-Service: Von "Code committen" bis "Deployment", unter Einbeziehung eurer vorhandenen Tests (`test_orders.py`, `test_events.py`) und des Docker-Images aus Modul 7.

3. **Gesamtsystem demonstrieren.** Startet den vollständigen Stack (`docker compose up --build`) und führt eine vollständige End-to-End-Demonstration durch: Bestellung anlegen, REST-API über Swagger UI (`/docs`) zeigen, Ereignis in RabbitMQ nachweisen, Benachrichtigung im Notification-Service-Log zeigen.
