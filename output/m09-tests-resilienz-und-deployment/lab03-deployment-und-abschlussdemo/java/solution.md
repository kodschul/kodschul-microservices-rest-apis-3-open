# Lösung (Java-Spur): Deployment-Strategien, DevOps und Abschlussdemonstration
...
```mermaid
flowchart LR
    A[Code committen] --> B[CI: mvn test<br/>OrderControllerTest, OrderEventPublisherTest]
...
1. Swagger UI öffnen (`http://localhost:8081/swagger-ui.html`), `POST /orders` über "Try it out" ausführen.
...
```markdown
# Capstone-Zusammenfassung: Bestell-Plattform (Java-Spur)
...
Contract-first mit `order-api.yaml`, vollständige CRUD-API, validiert, dokumentiert über springdoc/Swagger UI, Client generiert (Modul 2/3).
...
Multi-Stage-Dockerfiles, vollständiger Stack über Docker Compose mit Healthchecks (Modul 7/8).
