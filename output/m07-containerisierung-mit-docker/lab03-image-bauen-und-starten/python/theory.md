...
# Lab 3 (Python-Spur): Image bauen und einzeln starten
...
Da die Python-Lösung `host="localhost"` fest im Code hinterlegt hat (Modul 6), muss der Container den Host-Namen `host.docker.internal` aus derselben Umgebungsvariable lesen können. Ergänzt dazu in `app/events.py` bzw. `app/consumer.py` eine Umgebungsvariable statt des festen Strings:
